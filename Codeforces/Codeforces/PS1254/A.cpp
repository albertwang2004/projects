...
string ret = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
int A[H2][H2];
...
void read() {
    ...
    int thresh = rice%K;
    int need = rice/K+1;
    //cout << thresh << " " << need << endl;
    F0(i,N) {
        if (i&1) {F0(j,M) {
            if ((currrice==(need-(int)(currchicken>=thresh))) && (currchicken!=K-1))
                {currchicken++; currrice=0;}
            if (A[i][j]==-1) currrice++;
            A[i][j]=currchicken;
