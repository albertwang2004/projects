...
#define tc(T, ...) template <class T, class __VA_ARGS__>
...
tc(S,T) bool updmin(S &a, T b) {
    S B = (S)b;
    if (B < a) {a = B; return 1;}
    return 0;
}

tc(S,T) bool updmax(S &a, T b) {
    S B = (S)b;
    if (B > a) {a = B; return 1;}
    return 0;
}

tc(S,T) S min(S a, T b) {
    S c = a; updmin(c, b); return c;
}

tc(S,T) S max(S a, T b) {
...
void read() {
    ...
    if (snipe) cout << 1 << endl;
    else cout << max(2,(X+best-1)/best) << endl;
}
