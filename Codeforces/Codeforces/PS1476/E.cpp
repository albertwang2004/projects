...
void read() {
    N=rd(); M=rd(); K=rd();
    F0(i,N) {
        string sin; cin >> sin; P.pb(sin);
        revv[sin]=i;
    }
    F0(i,M) {
        string sin; cin >> sin; S.pb(sin); MT.pb(rd()-1);
    }
    ...
    F0(i,M) {
        ...
        F0(j, 1<<K) {
            ...
            if (revv.find(pat)!=revv.end()) {
                int idx = revv[pat];
                if (idx==need) ok=1;
                else {adj[need].pb(idx);
                 rev[idx].pb(need);}
            }
        }
        if (!ok) {
	...
	for (int i = 0; i < N; i++) for (int j : adj[i]) if (idx[j] <= idx[i]) {
		cout << "NO" << endl;// topological sort wasn't valid
		return;
	}
	cout << "YES" << endl;
