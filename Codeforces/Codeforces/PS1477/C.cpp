...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 0; i < N; i++) {
            bruh.ins(mp(mp(rd(),rd()),i+1));
        }

        pair<pll,int> best = *bruh.begin();
        ll longest = 0;
        pair<pll,int> curr = *bruh.begin();
        while (bruh.size()) {
            bruh.erase(curr);
            cout << curr.s << " ";
            longest = 0;
            for (auto xx : bruh) {
                pll x = xx.f;
                ll d = (x.f-curr.f.f)*(x.f-curr.f.f) + (x.s-curr.f.s)*(x.s-curr.f.s);
                if (d>longest) {
                    best = xx;
                    longest = d;
                }
