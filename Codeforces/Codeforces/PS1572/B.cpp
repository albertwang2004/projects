...
int hit(int i) {
    ...
    if (!A[i]&!A[i+1]&!A[i+2]) return 1;
    A[i]=A[i+1]=A[i+2]=A[i]^A[i+1]^A[i+2];ret.pb(i);
    return 1;
}
int smooth(int i) {
    if (i>=N-2) return 1;
    if (A[i]^A[i+1]^A[i+2]==0) hit(i);
    if (A[i]==0) return 0;
    if (!A[i+1]|!A[i+2]) hit(i);
    if (A[i+1]&A[i+2]) return smooth(i+2)*hit(i);
    return 1;
}
void pleaseworkgoddammit() {
    int best = 0;
    F0(i,N-2) if (smooth(i)) if (i>best) {best=i; i=-1;} else break;
}
void read() {
    int S = 0;
    ret.clear();
    N=rd(); F0(i,N) {A[i]=rd(); S+=A[i];}
    if (S&1) {cout << "NO" << endl; return;}
    pleaseworkgoddammit();
    pleaseworkgoddammit();
    pleaseworkgoddammit();
    pleaseworkgoddammit();
    ...
    cout << "YES" << endl;
    cout << ret.size() << endl;
    for (int i : ret) cout << i+1 << " "; cout << endl;
}
