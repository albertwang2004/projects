...
void read() {
    ...
    DP[0][0] = 0;
    F1(i,N) F0(j,2*H3) {
        if (j>=A[i]) updmin(DP[i][j-A[i]],DP[i-1][j]);
        else updmin(DP[i][0],DP[i-1][j]+A[i]-j);
        if (j+A[i]<2*H3) updmin(DP[i][j+A[i]],max(DP[i-1][j],j+A[i]));
    }
    ll ret = LNF;
    F0(j,2*H3) updmin(ret,DP[N][j]);
    cout << ret << endl;
}
