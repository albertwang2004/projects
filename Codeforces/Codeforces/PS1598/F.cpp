...
void read() {
    ...
    F1(i,(1<<N)-1) {
        int best = -50000000;
        F0(j,N) if (i&(1<<j)) {
            bruh[i].f=bruh[i^(1<<j)].f+L[j];
            int chng = OC[j][2*O-bruh[i^(1<<j)].f];
            int upd = DP[i^(1<<j)]+chng;
            updmax(ret,upd);
            if (bruh[i^(1<<j)].f+I[j]>=O)
                updmax(best,upd);
        }
        DP[i]=best;
    }
