...
void ds(char* v) { cout << v << " "; }
tcc(S,T) ds(pair<S,T> v) {cout << v.f << " " << v.s << endl;}
...
void read() {
    ...
    good.pb({1000000069,1000000069});
    srt(good); // increasing a
    srt(bad); // increasing s
    auto L = good.begin(); // inserting after
    auto R = L+1;
    FE(x,bad) {
        while (x.f >= R->s) {++L;++R;}
        if (debug) cout << "deciding (" << x.f << " " << x.s << ") on (" << L->f << " " << L->s << ") and (" << R->f << " " << R->s << ")" << endl;
        if ((L->s <= x.f) && (x.s <= R->f)) ugly.pb(x);
    }
    int curr = -69;
    int ans = good.size()-2;
    srtc(ugly,cmp);
    ...
    FE(x,ugly) if (x.f >= curr) {curr = x.s; ++ans;}
    ds(ans);
}
